...
import { useFollowing, useFollowStatus, useToggleFollow } from "@/hooks/use-follows";
...
export default function FollowingPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const { data: following, isLoading } = useFollowing(username);
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-xl mx-auto px-4 py-8">
        ...
        {!isLoading && following?.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-12">Not following anyone yet.</p>
        )}
