  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={cn(
          "flex flex-col items-center gap-0.5 px-2.5 py-2 rounded-xl border text-xs font-semibold self-center flex-none",
          redesign.hasUpvoted ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground",
        )}
      >
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            autoFocus
            className="w-full h-11 pl-10 pr-4 rounded-xl border border-input bg-card text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring text-sm"
          />
        </div>
        ...
        {q.trim().length >= 2 && !isLoading && results?.length === 0 && (
          <p className="text-center text-muted-foreground text-sm py-10">
            No redesigns found for &ldquo;{q}&rdquo;.
          </p>
        )}
        ...
        {q.trim().length < 2 && (
          <p className="text-center text-muted-foreground text-sm py-10">Type at least 2 characters to search.</p>
        )}
      </div>
    </div>
  );
